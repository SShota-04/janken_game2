...
window.onload = function () {
	...
	//クリック音読み込み
	const clickSndUrl = "sakana-haneru.mp3";						//game.htmlからの相対パス
	game.preload([clickSndUrl]); 				//データを読み込んでおく

	//sakana画像
	const sakanaImgUrl = "img/sakana2.png";						//game.htmlからの相対パス
	game.preload([sakanaImgUrl]);					//データを読み込んでおく
	...
	game.onload = function () {					//ロードが終わった後にこの関数が呼び出されるので、この関数内にゲームのプログラムを書こう
		...
		game.onenterframe = function () {
			if (state == 0) { 							//state=0のとき、初期セット状態(Pointの状態を０にして)
				sakanaImg.x = -200;						//sakanaのｘ座標を指定
				sakanaImg.y = 100;						//sakanaのy座標を指定
				point = 0;  							//point初期化
			...
			if (state == 4.1) {							//状態４．１ 移動状態
				sakanaImg.x += 10;						//ただ移動します
			}
			if (state == 5) {							//状態５（point１２以上から）　 ｙ軸が毎フレーム毎に変化する
				sakanaImg.x += 20;						//移動します。
				sakanaImg.y = Math.random() * 400;			//ｙ座標の位置を枚フレーム毎にランダム決定
			}
